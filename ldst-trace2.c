...
static struct qemu_plugin_scoreboard *block_start_time_score;
...
static void vcpu_update_clock_block(unsigned int cpu_index, void *udata) {
  if (!qemu_plugin_log_is_enabled()) {
    return;
  }

  uint64_t block_size = (uint64_t)(uintptr_t)udata;
  uint16_t start_time = __atomic_fetch_add(&logical_clock, block_size, __ATOMIC_SEQ_CST);
  uint64_t *local_storage = qemu_plugin_scoreboard_find(block_start_time_score,cpu_index);
  *local_storage = start_time;
}
...
static void vcpu_mem(unsigned int cpu_index, qemu_plugin_meminfo_t info,
  ...
  LogRecord record;
  uint64_t offset = (uint64_t)(uintptr_t)udata;
  uint64_t *start_time_ptr = qemu_plugin_scoreboard_find(block_start_time_score, cpu_index);
  uint64_t precise_time = *start_time_ptr + offset;
  ...
  record.logical_clock = precise_time;
...
static void vcpu_tb_trans(qemu_plugin_id_t id, struct qemu_plugin_tb *tb) {
  ...
  if (n_insns > 0) {
    ...
    qemu_plugin_register_vcpu_insn_exec_cb(insn, vcpu_update_clock_block, QEMU_PLUGIN_CB_NO_REGS, (void*)(uintptr_t) n_insns);
  ...
  for (size_t i = 0; i < n_insns; i++) {
    ...
    qemu_plugin_register_vcpu_mem_cb(insn, vcpu_mem, QEMU_PLUGIN_CB_NO_REGS,
                                     QEMU_PLUGIN_MEM_RW, (void*)(uintptr_t)i);
    qemu_plugin_register_vcpu_insn_exec_inline_per_vcpu(insn,QEMU_PLUGIN_INLINE_ADD_U64,insn_count_entry,1);
  }
}
...
static void plugin_exit(qemu_plugin_id_t id, void *p) { 
  ...
  qemu_plugin_scoreboard_free(block_start_time_score); 
}
...
QEMU_PLUGIN_EXPORT int qemu_plugin_install(qemu_plugin_id_t id,
  ...
  block_start_time_score = qemu_plugin_scoreboard_new(sizeof(uint64_t));
